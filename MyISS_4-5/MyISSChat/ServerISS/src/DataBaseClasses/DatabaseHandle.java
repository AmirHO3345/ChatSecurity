...
public class DatabaseHandle {
    ...
    public DatabaseHandle() {
        try {
            ...
            String URL ="jdbc:mysql://localhost:3306/db_chats?useSSL=false" ;
        ...
        try {
            ...
            if(LoginResult.next()) {
                DataResult.put("Data" , new JSONObject()
                    ...
                    .put("PublicKey" , LoginResult.getString("publicKey"))
                    .put("PrivateKey" , LoginResult.getString("privateKey")));
            } else
    ...
    public JSONObject GetUserDataByPhone(long Phone) {
        JSONObject DataResult = this.GetUserByPhone(Phone) ;
        if(DataResult.has("Data")) {
            DataResult = DataResult.getJSONObject("Data") ;
            DataResult.remove("UserID") ;
            DataResult.remove("PrivateKey") ;
            return new JSONObject().put("Data" , DataResult) ;
        }
        return DataResult ;
    }

    public JSONObject SetUserData(long Phone , String Password
            , String UserName) {
        JSONObject DataResult = new JSONObject() ;
        try {
    ...
    public JSONObject SetServerInfo(String ServerDomain , String PublicKeyServer
        ...
        try {
            ...
            int RegisterResult ;
            RegisterResult = RegisterStatement.executeUpdate(
                    "insert into server (`domain`, `publicKey` , `privateKey`) VALUES" +
                            "(" + "'" + ServerDomain + "'" + "," +
                            "'" + PublicKeyServer + "'" + "," + "'" + PrivateKeyServer + "'" + ");") ;
            if(RegisterResult > 0) {
                DataResult.put("Data" , "Done") ;
            }
        } catch (Exception e) {
            DataResult.put("DB_Error" , "Something Wrong With DB") ;
        }
    ...
    public JSONObject SetCertificate(String Certificate , Date CreateDate
            , Date ExpireDate , long UserPhone) {
        JSONObject DataResult = new JSONObject() ;
        try {
            JSONObject UserData = new JSONObject(this.GetUserByPhone(UserPhone).toString()) ;
            if(UserData.has("Data")) {
                int UserID = UserData.getJSONObject("Data").getInt("UserID") ;
                Statement RegisterStatement = this.DBConnect.createStatement() ;
                int RegisterResult ;
                RegisterResult = RegisterStatement.executeUpdate(
                        "insert into certificate (`certificate` , `createDate` , `expireDate` , `userId`) VALUES" +
                                "(" + "'" + Certificate + "'" + "," + "'" + new java.sql.Date(CreateDate.getTime()).toString() + "'"
                                + "," + "'" + new java.sql.Date(ExpireDate.getTime()).toString() + "'" + "," + UserID + ");") ;
                if(RegisterResult > 0) {
                    DataResult.put("Data" , "Done") ;
                }
            } else {
                DataResult.put("ErrorData" , "Phone Number Is Wrong") ;
            }
        } catch (Exception e) {
            DataResult.put("DB_Error" , "Something Wrong With DB") ;
        }
    ...
    public JSONObject SetUserKey(int UserID
            , String PublicKeyClient , String PrivateKeyClient) {
        JSONObject DataResult = new JSONObject() ;
        try {
            Statement RegisterStatement = this.DBConnect.createStatement() ;
            int RegisterResult = RegisterStatement.executeUpdate(
                    "UPDATE users U SET U.publicKey='" +
                    PublicKeyClient + "'," +
                    "U.privateKey='" + PrivateKeyClient + "' " +
                    "WHERE U.id=" + UserID + " ;") ;
            if(RegisterResult > 0) {
                DataResult.put("Data" , "Done") ;
            } else
    ...
    public JSONObject GetServerKeys(String ServerDomain) {
        JSONObject DataResult = new JSONObject() ;
        try {
            Statement GetStatement = this.DBConnect.createStatement() ;
            ResultSet GetKeysResult = GetStatement.executeQuery("" +
                    "select * from server S WHERE S.domain=" + "'" + ServerDomain + "'" + ";");
            if(GetKeysResult.next()) {
                DataResult.put("Data", new JSONObject()
                        .put("PublicKey" , GetKeysResult.getString("publicKey"))
                        .put("PrivateKey" , GetKeysResult.getString("privateKey"))) ;
            }
        } catch (Exception e) {
            DataResult.put("DB_Error" , "Something Wrong With DB") ;
        }
    ...
    public JSONObject GetUserCertificate(String UserName , long Phone) {
        JSONObject DataResult = new JSONObject() ;
        try {
            JSONObject UserData = this.GetUserByPhone(Phone) ;
            if(UserData.has("Data")) {
                UserData = UserData.getJSONObject("Data") ;
                if(UserName.equals(UserData.getString("UserName"))) {
                    int UserID = UserData.getInt("UserID") ;
                    Statement GetStatement = this.DBConnect.createStatement() ;
                    ResultSet GetKeysResult = GetStatement.executeQuery("" +
                            "select * from certificate C WHERE C.userId=" + UserID
                            + " ORDER by createDate DESC ;");
                    if(GetKeysResult.next()) {
                        Date ExpireDate = GetKeysResult.getDate("expireDate") ;
                        Date CurrentDate = new Date() ;
                        if(ExpireDate.getTime() > CurrentDate.getTime())
                            DataResult.put("Data", new JSONObject()
                                    .put("Certificate" , GetKeysResult.getString("certificate"))) ;
                        else
                            DataResult.put("CertificateExpire", "Last Certificate Is Expire") ;
                    } else
                        DataResult.put("Empty", "No Certificate Is There") ;
                } else {
                    DataResult.put("ErrorData", "The UserName Is Wrong") ;
                }
            } else {
                DataResult.put("ErrorData", "The Phone Number Is Wrong") ;
            }
        } catch (Exception e) {
            DataResult.put("DB_Error" , "Something Wrong With DB") ;
        }
        ...
        try {
            ...
            if(GetUserResult.next()) {
                DataResult.put("Data", new JSONObject()
                    ...
                    .put("PublicKey" , GetUserResult.getString("publicKey"))
                    .put("PrivateKey" , GetUserResult.getString("privateKey"))) ;
            }
        } catch (Exception e) {
            DataResult.put("DB_Error" , "Something Wrong With DB") ;
        }
