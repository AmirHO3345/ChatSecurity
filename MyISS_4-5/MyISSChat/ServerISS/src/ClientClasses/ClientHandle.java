...
public class ClientHandle extends Thread {
    ...
    public void SendResponse(JSONObject DataResponse) {
        try {
            if(this.IsHandCheck) {
                ...
                JSONObject EncryptJson = new JSONObject().put("EncryptionData" , new JSONObject()
                        .put("Data" , Base64.getEncoder()
                                .encodeToString(MessageEncryption)));
                this.ClientSend.write(EncryptJson.toString());
                System.out.println("Response : " + EncryptJson.toString());
            } else {
    ...
    public void SendMessage(JSONObject MSG) {
        ...
        try {
            ...
            this.ClientSend.newLine();
            this.ClientSend.flush();
            System.out.println("Message : " + EncryptJson.toString());
    ...
    public void SendSessionKey(JSONObject DataSend) {
        try {
            byte[] MessageEncryption = AES.encrypt(DataSend.toString() ,
                    this.ClientInfo.DataSecure.GetKey() ,
                    this.ClientInfo.DataSecure.GetIV()) ;
            JSONObject EncryptJson = new JSONObject().put("EncryptionData" , new JSONObject()
                    .put("Data" , Base64.getEncoder().encodeToString(MessageEncryption))) ;
            this.ClientSend.write(EncryptJson.toString());
            this.ClientSend.newLine();
            this.ClientSend.flush();
            System.out.println("Session : " + EncryptJson.toString());
            this.Logger(LoggerSate.SessionKey , DataSend.toString());
            System.out.println("\n\n\n");
        } catch (Exception e) {
            System.out.println("Error In SendMessage");
        }
    ...
    private void Logger(LoggerSate logState , String JsonData) {
        ...
        switch (logState) {
            ...
            case Message :
                ...
                break ;
            case SessionKey :
                System.out.println("Receive SessionKey" + JsonLog.toString());
                break ;
        }
    }
    ...
    private String ReceiveEncryptionWithAccount(String DataEncryption) {
        ...
        String DataEncrypt = DataJson.getString("Data") ;
        if(DataJson.has("DigitalSignature")) {
            String DigitalSignatureRequest = DataJson.getString("DigitalSignature");
            if(!DigitalSignature.verifySignature(DataEncrypt , this.ClientInfo.GetPublicKey() ,
                    DigitalSignatureRequest)) {
                System.out.println("The Digital Signature Is Not Match");
                return "" ;
            } else {
                System.out.println("The Digital Signature Is Success");
            }
        }
    ...
    private void SendCertificate() {
        JSONObject CertificateData = new JSONObject()
                .put("CertificateServerResponse" ,
                        CA.Certificate2String(this.ClientServer.GetServerCertificate()));
        this.SendResponse(CertificateData);
    }
